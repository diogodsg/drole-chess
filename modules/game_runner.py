...
PIECE_NAME_MAP = {"q": "Rainha", "r": "Torre", "b": "Bispo", "n": "Cavalo"}
...
class GameRunner:
    ...
    def run(self):
        ...
        while not self.chess_game.board.outcome() and (
            self.player_time > 0 or not self.has_time
        ):
            i = i + 1
            print(f"Inside loop {i}")
            print(self.chess_game.board.outcome())
            print(self.chess_game.make_matrix())
            if self.has_time:
                self.display.display(
    ...
    def handle_frame(self):
        ...
        if self.player_turn:
            ...
            move = detect_movement(
                self.chess_game.make_matrix(), detected_board["main_board"]
            )
            ...
            if is_equal_state:
                ...
                if piece_captured:
                    ...
                    self.path_module.move_to_cemitery(
                        self.bot_move[2:4],
                        piece_captured.piece_type,
                        piece_captured.color,
                    )
                ...
                self.path_module.move_piece(
                    self.bot_move[:4], piece_to_move.piece_type != 2
                )
                self.path_module.move_piece(
                    self.chess_game.get_castle_counterpart(self.bot_move), False
                )
                self.last_timestamp = time.time()
